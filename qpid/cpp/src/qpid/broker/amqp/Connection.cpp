...
void Connection::process()
{
    ...
    while (event) {
        ...
        case PN_LINK_REMOTE_CLOSE:
            ...
            break;
        case PN_LINK_FLOW:
            doLinkFlowUpdated(pn_event_link(event));
            break;
        case PN_DELIVERY:
...
// the link credit has changed on the link, flush as many pending writes
// as possible
void Connection::doLinkFlowUpdated(pn_link_t *link)
{
    if (pn_link_is_sender(link)) {
        pn_delivery_t *current = pn_link_current(link);
        while (current && pn_delivery_writable(current)) {
            doDeliveryUpdated(current);
            pn_delivery_t *next = pn_link_current(link);
            if (current == next)
                // have not advanced, done
                break;
            current = next;
        }
    }
